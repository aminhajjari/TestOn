...
class MILK10kPipeline:
    ...
    def create_segmented_outputs(self, image: np.ndarray, mask: np.ndarray) -> Dict[str, np.ndarray]:
        """Create only colored overlay for ConceptCLIP"""
        outputs = {}
    ...
    def classify_segmented_image(self, segmented_outputs: Dict[str, np.ndarray]) -> Dict:
        """Classify using local ConceptCLIP on colored overlay only"""
        try:
            if 'colored_overlay' not in segmented_outputs:
                print("Error: Colored overlay not found")
                return {}
            
            seg_image = segmented_outputs['colored_overlay']
            if seg_image is None or seg_image.size == 0:
                print("Error: Invalid colored overlay image")
                return {}
    ...
    def process_dataset(self, test_mode: bool = False, max_images: int = 5) -> Dict:
        ...
        for img_path in tqdm(image_files, desc="Processing MILK10k images"):
            try:
                ...
                if 'colored_overlay' in segmented_outputs:
                    seg_image = segmented_outputs['colored_overlay']
                    if seg_image is not None:
                        output_path = conceptclip_dir / "colored_overlay.png"
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH = "/project/def-arashmoh/shahab33/XAI/MILK10k_Training_Input/outputs_test"
